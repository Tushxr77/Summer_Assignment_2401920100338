# 🚀 Summer Assignment 2026 - DSA & OOPS

## 👨‍🎓 Student Information

| Details | Information |
|----------|------------|
| Name | Tushar Sharma |
| Roll Number | 2401920100338 |
| Branch | CSE |
| Language | Java |

---

## 📚 Assignment Overview

This repository contains solutions for the **4-Week DSA & OOPS Summer Assignment**.

### Daily Targets
- Solve 3 LeetCode Problems Daily
- Complete 1 OOPS Assignment Weekly
- Maintain Daily GitHub Commits
- Follow Proper Repository Structure

---

# 📈 Progress Dashboard

## Week 1

| Topic | Status | Quick Access |
|--------|--------|-------------|
| Day 1 - Arrays Basics | ✅ Completed | [View Folder](./Week_1/Day_1) |
| Day 2 - Prefix Sum & Sliding Window | ✅ Completed | [View Folder](./Week_1/Day_2) |
| Day 3 - Two Pointer Problems | ✅ Completed | [View Folder](./Week_1/Day_3) |
| Day 4 - Matrix Problems | ✅ Completed | [View Folder](./Week_1/Day_4) |
| Day 5 -  String Basics |  ✅ Completed | [View Folder](./Week_1/Day_5) |
| OOPS Assignment | ✅ Completed | [View Folder](./Week_1/OOPS) |

### Completed Problems

#### Day 1
- Two Sum
- Remove Duplicates from Sorted Array
- Best Time to Buy and Sell Stock

#### Day 2
- Maximum Subarray
- Contains Duplicate
- Maximum Average Subarray I

#### Day 3
- Move Zeroes
- Squares of a Sorted Array
- Container With Most Water

#### Day 4
- Matrix Diagonal Sum
- Reshape the Matrix
- Spiral Matrix

#### Day 5
- Valid Palindrome
- Reverse String
- Longest Common Prefix
---
### Week 1 OOPS Assignment

- Topic: Interface
- Status: ✅ Completed
- Folder: [View OOPS Solution](./Week_1/OOPS)

## Week 2

| Topic | Status | Quick Access |
|--------|--------|-------------|
| Day 1 -  Hashing in Strings |  ✅ Completed | [View Folder](./Week_2/Day_1) |
| Day 2 -  Sliding Window on Strings | ✅ Completed | [View Folder](./Week_2/Day_2) |
| Day 3 | ⏳ Pending | [View Folder](./Week_2/Day_3) |
| Day 4 | ⏳ Pending | [View Folder](./Week_2/Day_4) |
| Day 5 | ⏳ Pending | [View Folder](./Week_2/Day_5) |
| OOPS Assignment | ⏳ Pending | [View Folder](./Week_2/OOPS) |

### Day 1
- Valid Anagram
- First Unique Character in a String
- Ransom Note
  
### Day 2
- Longest Substring Without Repeating Characters
- Permutation in String
- Find All Anagrams in a String
---

## Week 3

| Topic | Status | Quick Access |
|--------|--------|-------------|
| Day 1 | ⏳ Pending | [View Folder](./Week_3/Day_1) |
| Day 2 | ⏳ Pending | [View Folder](./Week_3/Day_2) |
| Day 3 | ⏳ Pending | [View Folder](./Week_3/Day_3) |
| Day 4 | ⏳ Pending | [View Folder](./Week_3/Day_4) |
| Day 5 | ⏳ Pending | [View Folder](./Week_3/Day_5) |
| OOPS Assignment | ⏳ Pending | [View Folder](./Week_3/OOPS) |

---

## Week 4

| Topic | Status | Quick Access |
|--------|--------|-------------|
| Day 1 | ⏳ Pending | [View Folder](./Week_4/Day_1) |
| Day 2 | ⏳ Pending | [View Folder](./Week_4/Day_2) |
| Day 3 | ⏳ Pending | [View Folder](./Week_4/Day_3) |
| Day 4 | ⏳ Pending | [View Folder](./Week_4/Day_4) |
| Day 5 | ⏳ Pending | [View Folder](./Week_4/Day_5) |
| OOPS Assignment | ⏳ Pending | [View Folder](./Week_4/OOPS) |

---

# 🛠 Technologies Used

- Java
- LeetCode
- Git
- GitHub

---

# 🎯 Goal

To improve problem-solving skills, strengthen DSA concepts, and build strong Object-Oriented Programming fundamentals through consistent coding practice.

---

⭐ Repository maintained with daily commits and structured submissions as per college guidelines.
